...
interface AddHashDialogProps {
    isOpen: boolean;
    onClose?: () => void;
    redisKey: string;
    onConfirm?: (items: [string, string][]) => void;
}

export default function AddHashDialog({ isOpen, onClose, redisKey, onConfirm }: AddHashDialogProps) {
    ...
    const handleItemChange = (index: number, value: [string, string]) => {
        console.log(value);
    ...
    const handleConfirm = () => {
        const nonEmptyItems = items.filter(([key, value]) => key.trim() !== '' && value.trim() !== '');
        redisCommands.hashAddItems(redisKey, nonEmptyItems, null).then(() => {
            reset();
    ...
    return (
        <CustomDialog isOpen={isOpen} onClose={handleCancel} title={"Hash Add Item(s)"}>
